...
extern size_t total_bandwidth;
extern Bid crashed_id;
...
Bucket ORAM::ReadBucket(int index) {
    total_bandwidth += _storeBlockSize;
...
void ORAM::finilize(bool find, Bid& rootKey, int& rootPos) {
    ...
    unsigned int i = 0;
    int total_size = cache.size(), cur_size = 0;
...
void ORAM::save_data(FILE *f_out)
{
    size_t size;

    this->store->save_data(f_out);
    fwrite(&this->depth, sizeof(this->depth), 1, f_out);
    fwrite(&this->blockSize, sizeof(this->blockSize), 1, f_out);
    Utilities::save_map_data(f_out, this->cache);
    Utilities::save_vector_data(f_out, this->leafList);
    Utilities::save_vector_data(f_out, this->readviewmap);
    Utilities::save_vector_data(f_out, this->writeviewmap);

    size = this->modified.size();
    fwrite(&size, sizeof(size), 1, f_out);
    for (auto &itr:this->modified)
    {
        Utilities::save_bid(f_out, itr);
    }

    fwrite(&this->readCnt, sizeof(this->readCnt), 1, f_out);
    fwrite(this->key.data(), sizeof(byte_t), this->key.size(), f_out);
    fwrite(&this->plaintext_size, sizeof(this->plaintext_size), 1, f_out);
    fwrite(&this->bucketCount, sizeof(this->bucketCount), 1, f_out);
    fwrite(&this->clen_size, sizeof(this->clen_size), 1, f_out);
    fwrite(&this->batchWrite, sizeof(this->batchWrite), 1, f_out);
}

void ORAM::load_data(FILE *f_in)
{
    size_t size;

    this->store->load_data(f_in);
    fread(&this->depth, sizeof(this->depth), 1, f_in);
    fread(&this->blockSize, sizeof(this->blockSize), 1, f_in);

    for (auto &itr:this->cache)
        delete itr.second;
    this->cache.clear();
    Utilities::load_map_data(f_in, this->cache);

    this->leafList.clear();
    Utilities::load_vector_data(f_in, this->leafList);

    this->readviewmap.clear();
    Utilities::load_vector_data(f_in, this->readviewmap);

    this->writeviewmap.clear();
    Utilities::load_vector_data(f_in, this->writeviewmap);

    this->modified.clear();
    fread(&size, sizeof(size), 1, f_in);
    for (size_t i = 0; i < size; i++)
    {
        Bid b = Utilities::load_bid(f_in);
        this->modified.emplace(b);
    }

    fread(&this->readCnt, sizeof(this->readCnt), 1, f_in);
    fread(this->key.data(), sizeof(byte_t), this->key.size(), f_in);
    fread(&this->plaintext_size, sizeof(this->plaintext_size), 1, f_in);
    fread(&this->bucketCount, sizeof(this->bucketCount), 1, f_in);
    fread(&this->clen_size, sizeof(this->clen_size), 1, f_in);
    fread(&this->batchWrite, sizeof(this->batchWrite), 1, f_in);
}
