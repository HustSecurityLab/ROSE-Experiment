...
void Horus::insert(string keyword, int ind) {
    crashed_id = createBid(1, "abc", 129);
...
int Horus::generatePosition(string keyword, int updt_cnt, int src_cnt, int acc_cnt) {
    ...
    SHA256(key,32,hash_buf);
    memcpy(&result, hash_buf, 4);
...
void Horus::DumpData()
{
    FILE * f_out = fopen("Horus-Data.dat","wb");

    Utilities::save_map_data(f_out, this->UpdtCnt);
    Utilities::save_map_data(f_out, this->SrchCnt);
    Utilities::save_map_data(f_out, this->Access);
    Utilities::save_map_data(f_out, this->LastIND);
    Utilities::save_map_data(f_out, this->latestUpdatedCounter);
    Utilities::save_int(f_out, this->maxSize);

    this->OMAP_updt->save_data(f_out);
    this->ORAM_srch->save_data(f_out);
...
void Horus::LoadData()
{
    FILE * f_in = fopen("Horus-Data.dat","rb");

    this->UpdtCnt.clear();
    this->SrchCnt.clear();
    this->Access.clear();
    this->LastIND.clear();
    this->latestUpdatedCounter.clear();

    Utilities::load_map_data(f_in, this->UpdtCnt);
    Utilities::load_map_data(f_in, this->SrchCnt);
    Utilities::load_map_data(f_in, this->Access);
    Utilities::load_map_data(f_in, this->LastIND);
    Utilities::load_map_data(f_in, this->latestUpdatedCounter);
    this->maxSize = Utilities::load_int(f_in);

    this->OMAP_updt->load_data(f_in);
    this->ORAM_srch->load_data(f_in);
