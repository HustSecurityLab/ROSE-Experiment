...
void PRFORAM::load_data(FILE *f_in)
{
    this->store->load_data(f_in);
    fread(&this->depth, sizeof(this->depth), 1, f_in);
    fread(&this->blockSize, sizeof(this->blockSize), 1, f_in);

    for (auto &itr:this->stash)
        delete itr.second;
    this->stash.clear();
    Utilities::load_map_data(f_in, this->stash);

    this->viewmap.clear();
    Utilities::load_vector_data(f_in, this->viewmap);
