...
int AVLTree::RandomPath() {
    ...
    if (val < 0 )
        val = -val;
    val =  dis(mt);
    val = val % ((int)((pow(2, floor(log2(_maxSize / Z)) + 1) - 1) / 2));*/

    return val;
}

void AVLTree::save_data(FILE *f_out)
{
    Utilities::save_int(f_out, _maxSize);
    this->oram->save_data(f_out);
}

void AVLTree::load_data(FILE *f_in)
{
    _maxSize = Utilities::load_int(f_in);
    this->oram->load_data(f_in);
}
